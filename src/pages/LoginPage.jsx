...
export default function SignInPage() {
  ...
  const [error, setError] = useState(null); // Fixed: uncommented and properly declared
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Optional: Verify user role from database (simplified version)
      if (userId) {
        try {
          ...
          if (roleError) {
            ...
            // Continue anyway - don't block login for role fetch error
          } else if (userRole && userRole.role !== selectedRole) {
          ...
          console.warn('Role verification failed, continuing with login:', roleErr);
  ...
  return (
    <div className={`h-screen w-full overflow-hidden relative flex items-center justify-center p-2 sm:p-4 ${isDark ? 'dark' : ''}`}>
      ...
      <div className="z-10 w-full max-w-7xl h-full flex items-center justify-center">
        <div className="w-full max-w-6xl overflow-hidden rounded-2xl sm:rounded-3xl lg:rounded-4xl shadow-2xl">
              ...
              <div className="relative z-10 flex flex-col h-full p-4 sm:p-6 lg:p-8">
                ...
                <div className="flex-shrink-0 mb-4 lg:mb-6">
                  <div className="mb-3 lg:mb-4">
                    <div className={`flex items-center text-sm sm:text-base lg:text-lg font-semibold uppercase ${isDark ? 'text-white' : 'text-slate-700'}`}>
                ...
                <div className="flex-1 flex flex-col min-h-0">
                  <div className="flex flex-col space-y-2.5 lg:space-y-3 overflow-hidden">
                    {roles.map((role) => (
                      <div
                        key={role.id}
                        className={`role-card p-3 lg:p-4 rounded-lg lg:rounded-xl flex-shrink-0 ${
                          selectedRole === role.id ? 'selected' : ''
                        }`}
                        onClick={() => setSelectedRole(role.id)}
                      >
                        <div className="flex items-center">
                          <div className={`mr-3 lg:mr-4 flex h-9 w-9 lg:h-10 lg:w-10 items-center justify-center rounded-lg backdrop-blur-sm flex-shrink-0 ${
                          ...
                          <div className="flex-1 min-w-0">
                            <div className={`font-semibold text-sm lg:text-base truncate ${
                              selectedRole === role.id ? 'text-white' : isDark ? 'text-white' : 'text-slate-800'
                            }`}>
                              {role.title}
                            </div>
                            <div className={`text-xs lg:text-sm opacity-70 truncate ${
                              selectedRole === role.id ? 'text-white' : isDark ? 'text-white' : 'text-slate-600'
                            }`}>
                              {role.desc}
                            </div>
                          </div>
                          <div className={`w-3.5 h-3.5 lg:w-4 lg:h-4 rounded-full border-2 transition-all flex-shrink-0 ${
                            selectedRole === role.id 
                ...
                {selectedRole && (
                  <div className={`flex-shrink-0 mt-4 lg:mt-6 p-3 lg:p-4 rounded-lg lg:rounded-xl backdrop-blur-sm border ${
                    isDark 
                    ...
                    <p className={`text-xs lg:text-sm flex items-center ${isDark ? 'text-white' : 'text-slate-700'}`}>
                      ...
                      <span className="truncate">Selected: <span className="font-semibold">{roles.find(r => r.id === selectedRole)?.title}</span></span>
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="h-full flex items-center justify-center p-4 sm:p-6 lg:p-8">
                <div className="w-full max-w-md">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 lg:space-y-5">
                    {/* Email Field */}
                    <div>
                      <label htmlFor="email" className={`mb-1 sm:mb-2 block text-xs sm:text-sm font-medium uppercase ${isDark ? 'text-gray-300' : 'text-slate-700'}`}>
                        Email address
                      </label>
                      <div className="relative">
                        ...
                        <input
                          ...
                          className={`w-full pl-9 sm:pl-10 pr-3 py-2.5 sm:py-3 border rounded-lg focus:outline-none focus:ring-2 transition-colors ${
                            isDark
                    ...
                    {/* Password Field */}
                    <div>
                      <label htmlFor="password" className={`mb-1 sm:mb-2 block text-xs sm:text-sm font-medium uppercase ${isDark ? 'text-gray-300' : 'text-slate-700'}`}>
                        Password
                      </label>
                      <div className="relative">
                        <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                        ...
                        <input
                          ...
                          className={`w-full pl-9 sm:pl-10 pr-10 sm:pr-12 py-2.5 sm:py-3 border rounded-lg focus:outline-none focus:ring-2 transition-colors ${
                            isDark
                              ? 'bg-white/10 border-white/20 text-white placeholder-gray-400 focus:ring-blue-400 focus:border-blue-400'
                              : 'bg-white/70 border-slate-300 text-slate-900 placeholder-slate-500 focus:ring-blue-500 focus:border-blue-500'
                          }`}
                    ...
                    {/* Error Display */}
                    {error && (
                      <div className="text-red-500 text-xs sm:text-sm bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-2 sm:p-3">
                        {error}
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between text-xs sm:text-sm">
                      <label className={`flex items-center cursor-pointer ${isDark ? 'text-gray-300' : 'text-slate-600'}`}>
                        <input type="checkbox" className="mr-1.5 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4 rounded" />
                        Remember me
                      </label>
                    ...
                    <div className="relative mt-4 sm:mt-6">
                      ...
                      <div className="relative flex justify-center text-xs sm:text-sm">
                        <span className={`px-2 ${isDark ? 'bg-transparent text-gray-300' : 'bg-transparent text-slate-500'}`}>
                          Or continue with
                        </span>
                      </div>
                    </div>
